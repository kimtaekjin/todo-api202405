...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                //securitu를 사용하면, 서버가 가동될 대 기본적으로 제공하는 여러가지 필터가 세팅이 되는데,
                //JwtAuthFilter를 먼저 배치해서, 이를 통과하면  인증이 완료가 되도록 처리
                .addFilterBefore(jwtAuthFilter,UsernamePasswordAuthenticationFilter.class)
                .authorizeHttpRequests(authorizeRequests ->
                        authorizeRequests

                                 // '/api/todos' 라는 요청이 post로 들어오고, Role 값이 ADmin인 경우 권한 검사 없이 허용
//                                .requestMatchers(HttpMethod.POST,"/api.todos").hasRole("ADMIN")


//                                .requestMatchers("/**").permitAll()); // 모든 요청에 대해 인증을 하지 않겠다.
